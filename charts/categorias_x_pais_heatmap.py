...
df = pd.read_csv("./dados_exterior.csv")

# Create a DataFrame for the heatmap
df_2dhist = pd.DataFrame(
    {
        pais: df[df["país"] == pais]["categoria"]
        .str.split(", ")
        .explode()
        .value_counts()
...
fig = px.imshow(
    df_2dhist,
    labels=dict(x="Categoria", y="País", color="Count"),
    x=df_2dhist.columns,
    y=df_2dhist.index,
...
chart = html.Div(
    children=[
        html.H3("Categorias x País", className=styles["h3"]),
